...
from gsplat.compression import SeqHevcCompressionCom
...
class AttributeCodecs:
    ...
    quats: CodecConfig = field(default_factory=lambda: CodecConfig("_skip_compression", "_skip_decompression"))
    ...
    shN: CodecConfig = field(default_factory=lambda: CodecConfig("_compress_combined_kmeans", "_decompress_combined_kmeans"))
...
class Runner:
    def __init__(
        ...
        if cfg.compression == "seq_hevc":
            self.compression_method = SeqHevcCompressionCom(**self.compression_cfg)
    ...
    def load_ply_sequences(
        ...
        print("📦 Available keys in splats:", list(splats.keys()))
    ...
    def compress(self,):
        ...
        # shape 변경 및 flatten
        shn = splats_videos["shN"].cpu().detach().numpy()  # [T, H, W, 15, 3]
        T, H, W, _, _ = shn.shape
        shn_flat = shn.reshape(T * H * W, -1)  # [N, 45]

        quats = splats_videos["quats"].cpu().detach().numpy().reshape(T * H * W, 4)  # [N, 4]

        combined = np.concatenate([shn_flat, quats], axis=1)  # [N, 49]
        np.save(os.path.join(compress_dir, "shN_quat_raw.npy"), combined)
