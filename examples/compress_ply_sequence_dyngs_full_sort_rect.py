...
from gsplat.compression import SeqHevcCompressionFullSortRect
...
class Runner:
    def __init__(
        ...
        if cfg.compression == "seq_hevc":
            self.compression_method = SeqHevcCompressionFullSortRect(**self.compression_cfg)
    ...
    def compress(self,):
        ...
        # 직접 compression_cfg 출력 (원본 설정 객체)  
        print("Configuration Dictionary:")  
        print(json.dumps(self.compression_cfg, indent=2))  
    ...
    def rasterize_splats(
        ...
        **kwargs,
        ...
        if splats is not None:
            ...
            # sh0 처리 - 형태 확인 및 필요시 재구성
            if sh0.dim() == 1 and sh0.shape[0] % 3 == 0:
                sh0 = sh0.reshape(-1, 3)
            
            # 구형 고조파 계수 채널 확인
            if shN.dim() == 3:
                sh_channels = shN.shape[1]  # 일반적으로 15
                
                # 채널 수를 기반으로 SH 차수 결정
                if 'sh_degree' not in kwargs:
                    # sh_degree = 3인 경우 16 채널 필요 (즉, 15 채널이면 sh_degree=3에 맞지 않음)
                    if sh_channels <= 4:
                        kwargs['sh_degree'] = 1
                    elif sh_channels <= 9:
                        kwargs['sh_degree'] = 2
            ...
            # 단순히 sh0과 shN을 연결
            colors = torch.cat([sh0, shN], 1)  # [N, K+1, 3] where K is the number of SH basis
