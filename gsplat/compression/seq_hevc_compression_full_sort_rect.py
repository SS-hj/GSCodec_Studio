...
@dataclass
class SeqHevcCompressionFullSortRect:
    ...
    def reorganize(self, splats_list: List[Dict]) -> Dict[str, Tensor]:
        ...
        if "shN" in padded_splats_videos and "quats" in padded_splats_videos and self.use_sort:
            ...
            try:
                ...
                # ===== 각 클러스터별 평균값 계산 =====  
                print("Step 3: Computing cluster averages...")  
                ...
                for c, indices in clusters.items():  
                    if not indices:  # 빈 클러스터 처리  
                        continue  
                    
                    # 이 클러스터에 속한 모든 요소의 평균 계산  
                    cluster_avg = {}  
                    for attr_name, data in first_frame_data.items():  
                        if len(indices) > 0:  
                            ...
                            if attr_name == "opacities":  
                                ...
                                cluster_avg[attr_name] = torch.zeros_like(data[0])  
                        ...
                        for x in range(side):  
                            ...
                            if 0 <= grid_y < self.grid_height and 0 <= grid_x < self.grid_width:  
                                ...
                                if flat_idx < max_pos and reverse_mapping[flat_idx] == -1:  
                                    padding_positions[c].append(flat_idx)  
    ...
    def deorganize(self, splats_videos: Dict[str, Tensor]) -> List[Dict]:
        """
        클러스터 평균값으로 채워진 2D 그리드를 원본 데이터로 복원
        """
        # 2D 그리드 -> 1D 변환 
        flat_splats_videos = {}
        for attr_name, splats_video in splats_videos.items():
        ...
        N_grid = flat_splats_videos["means"].shape[1]  # 그리드 전체 크기 (패딩 포함)
        device = flat_splats_videos["means"].device
        
        # 클러스터링 그리드 정보 확인
        if hasattr(self, 'cluster_grid_info') and isinstance(self.cluster_grid_info, dict):
            print("Restoring data using cluster grid mapping information...")
            
            # 1. 원본 데이터 크기 계산 (패딩 제외)
            # cluster_elements 값의 합을 통해 실제 데이터 수 계산
            original_elements = sum(self.cluster_grid_info.get("cluster_elements", {}).values())
            if original_elements <= 0:
                # 정보가 없으면 총 요소 수 사용
                original_elements = self.cluster_grid_info.get("total_elements_mapped", N_grid)
            
            print(f"Original data elements: {original_elements}, Grid size: {N_grid}")
            ...
            for c_str, pos in self.cluster_grid_info.get("cluster_positions", {}).items():
                c = int(c_str)
                if len(pos) >= 3:
                    # (y_start, x_start, side) 형식
                    cluster_positions[c] = (pos[0], pos[1], pos[2])
            
            # 3. 패딩 위치와 데이터 위치 식별
            valid_positions = set()  # 실제 데이터가 있는 위치
            grid_width = self.cluster_grid_info.get("grid_width", int(np.sqrt(N_grid)))
            
            # 클러스터별 원본 요소 및 위치 추적
            for c_str, count in self.cluster_grid_info.get("cluster_elements", {}).items():
                c = int(c_str)
                ...
                # 클러스터 내 실제 데이터 위치 기록 (순서대로)
                idx = 0
                for y in range(side):
                    for x in range(side):
                        if idx >= count:  # 실제 데이터 끝나면 중단
                            break
                        ...
                        # 유효 위치로 기록
                        valid_positions.add(flat_idx)
                        idx += 1
            
            print(f"Identified {len(valid_positions)} valid data positions out of {N_grid} total positions")
            
            # 4. 유효 위치 목록 정렬 (일관된 순서)
            valid_positions = sorted(list(valid_positions))
            ...
            for attr_name, tensor in flat_splats_videos.items():
                ...
                reordered = []
                for t in range(T):
                    frame = tensor[t]  # [N_grid, ...]
                    
                    # 유효 위치만 선택
                    valid_data = frame[valid_positions]
                    
                    if len(valid_data) < original_elements:
                        # 부족한 경우 패딩 (실제로는 발생하지 않아야 함)
                        print(f"Warning: Missing data - padding from {len(valid_data)} to {original_elements}")
                        pad_size = list(valid_data.shape)
                        pad_size[0] = original_elements - len(valid_data)
                        padding = torch.zeros(pad_size, dtype=valid_data.dtype, device=device)
                        valid_data = torch.cat([valid_data, padding], dim=0)
                    elif len(valid_data) > original_elements:
                        # 초과하는 경우 자르기
                        print(f"Warning: Extra data - trimming from {len(valid_data)} to {original_elements}")
                        valid_data = valid_data[:original_elements]
        ...
        # 블록 매핑 정보 확인
        elif hasattr(self, 'block_info') and isinstance(self.block_info, dict):
            # 기존 역매핑 처리 (블록 기반)
            inverse_mapping = self.block_info.get('inverse_mapping', list(range(N_grid)))
            inverse_tensor = torch.tensor(inverse_mapping, dtype=torch.long, device=device)
            
            if len(inverse_tensor) != N_grid:
                print(f"Warning: Inverse mapping length mismatch ({len(inverse_tensor)} != {N_grid})")
                inverse_tensor = torch.arange(N_grid, device=device)
            
            print("Applying inverse block mapping to restore original order...")
            reordered_data = {}
            for attr_name, tensor in flat_splats_videos.items():
                reordered = []
                for t in range(T):
                    frame = tensor[t]
                    restored = frame.index_select(0, inverse_tensor)
        ...
        # 트리맵/일반 매핑 정보 확인
            ...
            # 기존 트리맵 처리
            inverse_mapping = self.treemap_info.get('inverse_mapping', list(range(N_grid)))
            inverse_tensor = torch.tensor(inverse_mapping, dtype=torch.long, device=device)
            ...
            # 기존 매핑 처리
            inverse_mapping = self.mapping_info.get('inverse_mapping', list(range(N_grid)))
            inverse_tensor = torch.tensor(inverse_mapping, dtype=torch.long, device=device)
